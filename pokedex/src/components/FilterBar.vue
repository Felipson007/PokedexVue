<template>
  <div class="filter-bar dropdown" ref="dropdownContainer">
    ...
    <ul class="dropdown-menu" :class="{ show: dropdownOpen }" aria-labelledby="dropdownMenuButton">
      <li>
        <a class="dropdown-item" href="#" :class="{ active: selectedType === '' }" @click="selectType('')">
          Todos
        </a>
      </li>
      <li v-for="type in types" :key="type.name">
        <a class="dropdown-item" href="#" :class="{ active: selectedType === type.name }"
...
.dropdown-menu {
  ...
  /* Adiciona scroll vertical */
  width: 100%;
  /* Garante que o dropdown ocupe toda a largura do botão */
  left: 0;
  /* Alinha o dropdown ao início do botão */
  top: 100%;
  /* Coloca o dropdown exatamente abaixo do botão */
  border-radius: 0 0 8px 8px;
  /* Adiciona bordas arredondadas na parte inferior */
...
.dropdown-toggle {
  min-width: 200px;
  /* Define um tamanho mínimo para o botão */
  width: 100%;
...
@media (max-width: 768px) {
  ...
  .dropdown-menu {
    width: 100%;
    /* O dropdown ocupa a mesma largura do botão */
  }
}
</style>
